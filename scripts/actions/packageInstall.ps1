...
& "$windir\NeptuneDir\Scripts\initPowerShell.ps1"
$sys32 = [Environment]::GetFolderPath('System')
$safeModePackageList = "$sys32\safeModePackagesToInstall.neptunemodule"
...
"@
	...
	if ($failedPackages.Count -gt 0) {
		...
		if ($safeModeStatus) {
			Write-Host "Please report this, as there's no automatic fallbacks past Safe Mode." -ForegroundColor Magenta
...
if ($SafeMode) {
	...
	if ($matchedPackages.Count -le 0) {
		Write-Host "[ERROR] Safe Mode package list not found! Please report this to our GitHub." -ForegroundColor Red
		ExitSafeModePrompt
	}

	$packagesThatDontExist = $matchedPackages | ForEach-Object { if (!(Test-Path $_ -PathType Leaf)) { $_ } }
	if ($packagesThatDontExist) {
		Write-Host "[ERROR] Some Safe Mode packages weren't found. Please report this to our GitHub." -ForegroundColor Red
...
Would you like to restart your system into Safe Mode and try again? This process shouldn't take much time.

Please note that if you chose to disable Windows Defender, it may still remain enabled if you select 'No'. However, you can always try disabling it later in the Neptune folder.
"@

	if ((Read-MessageBox -Title "Neptune - Component Modification" -Body $body -Icon Question) -eq 'Yes') {
		SafeMode -Enable
		Restart
	}
...
function MakeRepairSource {
	...
	$srcPath = "%SystemRoot%\NeptuneDir\Packages\WinSxS"
